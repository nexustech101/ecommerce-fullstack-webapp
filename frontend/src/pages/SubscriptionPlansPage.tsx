...
export function SubscriptionPlansPage() {
  ...
  const canStartSubscription = Boolean(customer || (guest.name.trim() && guest.email.trim()));
  ...
  async function subscribe(planId: number) {
    ...
    try {
      const session = await api.createCheckoutSession({
        ...
        guest: customer ? undefined : guest.name && guest.email ? guest : undefined
      });
    ...
    <section className="plans-page">
      ...
      {customer ? (
        <div className="panel account-summary">
          ...
          <div>
            <strong>Subscribing as {customer.name}</strong>
      ...
      <div className="plan-grid">
        {plans.map((plan) => (
          <article className="plan-card" key={plan.id}>
            ...
            <button className="primary-button" onClick={() => subscribe(plan.id)} disabled={selectedPlanId === plan.id || !canStartSubscription}>
