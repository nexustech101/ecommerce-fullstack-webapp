...
from app.migration import MigrationRecord, apply_migrations, migration_status
...
def test_migration_runner_applies_and_records_versions_idempotently():
    ...
    assert [row["applied"] for row in before] == [False, False]
    assert first == ["0001", "0002"]
    assert second == []
    assert [row["applied"] for row in after] == [True, True]
    assert MigrationRecord.objects.count() == 2
